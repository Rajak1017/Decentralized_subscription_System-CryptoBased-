    ...
    <div className="min-h-screen py-12">
      <div className="container mx-auto px-4">
        ...
        {userSubscriptions.length === 0 ? (
          ...
          >
            <ElectricBorder style={{ borderRadius: 16 }}>
              <Card className="card-gradient p-12 max-w-md mx-auto h-full flex flex-col">
                <div className="w-20 h-20 bg-muted/20 rounded-full flex items-center justify-center mx-auto mb-6 flex-shrink-0">
                  <Plus className="h-10 w-10 text-muted-foreground" />
                </div>
                
                <h2 className="text-xl font-semibold mb-4 flex-shrink-0">No Subscriptions Yet</h2>
                ...
                {process.env.NODE_ENV === 'development' && (
                  <div className="text-xs text-muted-foreground mb-4 p-2 bg-muted/20 rounded flex-shrink-0">
                ...
                <Link to="/plans">
                  <ElectricBorder style={{ borderRadius: 8 }}>
                    <Button className="btn-gradient mt-auto flex-shrink-0">
                      Browse Plans
                    </Button>
                  </ElectricBorder>
                </Link>
          ...
          <div className="space-y-8">
            ...
            {filteredSubscriptions.length === 0 && searchQuery && (
              ...
              >
                <ElectricBorder style={{ borderRadius: 16 }}>
                  <Card className="card-gradient p-8 max-w-md mx-auto h-full flex flex-col">
                    <Search className="h-12 w-12 text-muted-foreground mx-auto mb-4 flex-shrink-0" />
                    <h3 className="font-semibold mb-2 flex-shrink-0">No Results Found</h3>
        ...
        {userSubscriptions.length > 0 && (
          ...
          >
            <ElectricBorder style={{ borderRadius: 16 }}>
              <Card className="card-gradient p-6 text-center h-full flex flex-col justify-center">
                <div className="text-2xl font-bold text-neon-green mb-1 flex-shrink-0">
                  {activeSubscriptions.length}
                </div>
                <div className="text-sm text-muted-foreground flex-shrink-0">Active</div>
              </Card>
            </ElectricBorder>
            
            <ElectricBorder style={{ borderRadius: 16 }}>
              <Card className="card-gradient p-6 text-center h-full flex flex-col justify-center">
                <div className="text-2xl font-bold text-muted-foreground mb-1 flex-shrink-0">
                  {inactiveSubscriptions.length}
                </div>
                <div className="text-sm text-muted-foreground flex-shrink-0">Inactive</div>
              </Card>
            </ElectricBorder>
            
            <ElectricBorder style={{ borderRadius: 16 }}>
              <Card className="card-gradient p-6 text-center h-full flex flex-col justify-center">
                <div className="text-2xl font-bold text-neon-cyan mb-1 flex-shrink-0">
                  {userSubscriptions.length}
                </div>
                <div className="text-sm text-muted-foreground flex-shrink-0">Total</div>
              </Card>
            </ElectricBorder>
            
            <ElectricBorder style={{ borderRadius: 16 }}>
              <Card className="card-gradient p-6 text-center h-full flex flex-col justify-center">
                <div className="text-2xl font-bold text-neon-purple mb-1 flex-shrink-0">
                  {userSubscriptions.reduce((sum, sub) => sum + parseFloat(sub.price), 0).toFixed(2)}
                </div>
                <div className="text-sm text-muted-foreground flex-shrink-0">Total Spent</div>
              </Card>
            </ElectricBorder>
          </motion.div>
        )}
